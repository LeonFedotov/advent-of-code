...
`
	...
	.split('\n')
	.map(l => {
		let [timestamp, message] = l.trim().split('] ')
		let [date, time] = timestamp.split(' ')
		return {
			date: new Date(timestamp.slice(1)),
...
let entry
const res = {}
while(entry = input.shift()) {
	let {message:msg, date} = entry
	res[msg] = res[msg] || [0, _.fill(Array(60), 0)]
	while(input[1] && !_.startsWith(input[0].message, 'Guard')) {
		let [total, timeTable] = res[msg]
		let [start, end] = [parseInt(input.shift().date.getMinutes(),10), parseInt(input.shift().date.getMinutes(), 10)]

		let duration = end - start
		res[msg] = [total+duration, timeTable.map((m, i) => i>=start && i<=start+duration?m+1:m)]
	}
}

_
	.chain(res)
	.map(([total, timeTable], key) => [total, key, timeTable.map(m => _.padStart(m, 2, '0')), Math.max(...timeTable)])
	.sort(([,,,a], [,,,b]) => b-a)
	.take(5)
	.thru(v => [['total', 'id', _.range(60).map((m) => _.padStart(m, 2, '0')), 'max minute sleep'],...v])
	.thru(table)
	.thru(console.log)
	.value()
//console.log(res)
