...
const visit = (x, y, id, steps) =>
	_.set(
		...
		`${x}:${y}`,
		{
			...
			visits: [..._.get(locations, `${x}:${y}.visits`, []), id],
			steps: [..._.get(locations, `${x}:${y}.steps`, []), steps]
		}
	)

input.forEach((path, idx) => {
	let x = 0, y = 0
	let len = 0
	path.forEach(move => {
		let dir = move.slice(0, 1)
		let dis = parseInt(move.slice(1), 10)
		let nx, ny
		switch(dir) {
			case 'R':
				nx = x+dis
...
_
	.chain(locations)
	.omit(['0:0'])
	.filter(({visits:v}) => _.uniq(v).length > 1)
	.map((s) => ({sum: _.sum(s.steps), ...s}))
	.sort(({sum:a }, {sum:b}) =>
		a-b
	)
	.thru(console.log)
