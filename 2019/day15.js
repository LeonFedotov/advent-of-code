...
const putPixel = (x, y, color) => {
	...
	ctx.point(x, y)
	ctx.bg(0, 0, 0)
	ctx.text(50, 2, `state: ${JSON.stringify({x, y, color})}`)
}
...
const keyIn = rl.keyIn.bind(rl, '', {hideEchoBack: true, mask: '', limit: '$<1-9>'})
...
while(res!=2) {
	...
	if(res == 2) {
		putPixel(...move(dir, pos).concat([colors.loot]))
	}
}
ctx.cursor.restore()
ctx.cursor.reset()
